...
from . import error as lucidity_error
...
class Template(object):
    ...
    def __init__(self, name, pattern, anchor=ANCHOR_START,
        ...
        *duplicate_placeholder_mode* determines how duplicate placeholders will
        ...
        extract the same value and raises :exc:`~lucidity_error.ParseError` if
        they do not.
    ...
    def expanded_pattern(self):
        ...
        Raise :exc:`lucidity_error.ResolveError` if pattern contains a reference
    ...
    def _expand_reference(self, match):
        ...
        if self.template_resolver is None:
            raise lucidity_error.ResolveError(
    ...
    def parse(self, path):
        ...
        Raise :py:class:`~lucidity_error.ParseError` if *path* is not
        ...
        if match:
            ...
            for key, value in sorted(match.groupdict().items()):
                ...
                if self.duplicate_placeholder_mode == self.STRICT:
                    if key in parsed:
                        if parsed[key] != value:
                            raise lucidity_error.ParseError(
    ...
    def format(self, data):
        ...
        Raise :py:class:`~lucidity_error.FormatError` if *data* does not
    ...
    def _format(self, match, data):
        ...
        try:
            ...
            raise lucidity_error.FormatError(
